...
class Content_Meta_Box {
	...
	public function render( $post ) {
		...
		<div class="alumni-content-fields">
			...
			<div id="alumni-terms-fields" class="alumni-terms-fields"<?php echo Post_Type::KIND_TERMS === $kind ? '' : ' style="display:none;"'; ?>>
				...
				<p>
					<label for="alumni_terms_font_color"><strong><?php esc_html_e( '本文全体の既定の文字色（任意）', 'alumni-core' ); ?></strong></label><br />
					...
					<label for="alumni_terms_font_color_text" class="screen-reader-text"><?php esc_html_e( '文字色のカラーコード', 'alumni-core' ); ?></label>
					<input type="text" id="alumni_terms_font_color_text" class="regular-text" value="<?php echo esc_attr( $font_color ); ?>" placeholder="#222222" readonly />
					<p class="description"><?php esc_html_e( '規約本文全体の既定色です。色を指定しない場合はテーマの標準色を使用します。段落や見出しなどを個別に色付けしたい場合は、上のブロックエディターで設定できます。', 'alumni-core' ); ?></p>
				</p>
				<p class="description">
	...
	public function save( $post_id, $post ) {
		...
		if ( Post_Type::KIND_PERSON_GREETING === $kind ) {
			...
			$font_color     = isset( $_POST['alumni_terms_font_color'] ) ? sanitize_hex_color( wp_unslash( $_POST['alumni_terms_font_color'] ) ) : '';
