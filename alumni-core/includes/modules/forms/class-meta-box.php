...
class Meta_Box {
	...
	private function render_field_row( $index, $field ) {
		$field = wp_parse_args( is_array( $field ) ? $field : array(), array(
			...
			'help_text' => '', 'placeholder' => '', 'options' => '', 'min_length' => 0, 'max_length' => 0, 'width' => '100', 'row_end' => true, 'textarea_rows' => 4, 'email_confirm' => false, 'email_confirm_label' => '', 'allowed_extensions' => 'pdf,doc,docx,xls,xlsx,jpg,jpeg,png', 'max_file_size_mb' => 5,
		) );
		$types = array(
		...
		<div class="alumni-form-admin-field">
			<div class="alumni-form-admin-grid">
				...
				<p><label><?php esc_html_e( '複数行テキストの高さ（行数）', 'alumni-core' ); ?><input type="number" min="2" max="50" step="1" name="alumni_form_fields[<?php echo esc_attr( $index ); ?>][textarea_rows]" value="<?php echo esc_attr( max( 2, min( 50, (int) $field['textarea_rows'] ) ) ); ?>" /></label><span class="description"><?php esc_html_e( 'textarea の表示高さです。複数行テキスト以外では無視されます。', 'alumni-core' ); ?></span></p>
			</div>
			<p><label><input type="checkbox" name="alumni_form_fields[<?php echo esc_attr( $index ); ?>][email_confirm]" value="1" <?php checked( ! empty( $field['email_confirm'] ) ); ?> /> <?php esc_html_e( 'メールアドレス確認用入力欄を表示する', 'alumni-core' ); ?></label><span class="description"> <?php esc_html_e( 'メールアドレス項目で使用します。送信時に入力値の一致を確認します。', 'alumni-core' ); ?></span></p>
			<p><label><?php esc_html_e( '確認用ラベル', 'alumni-core' ); ?><input type="text" class="widefat" name="alumni_form_fields[<?php echo esc_attr( $index ); ?>][email_confirm_label]" value="<?php echo esc_attr( $field['email_confirm_label'] ); ?>" /></label><span class="description"> <?php esc_html_e( '空欄の場合は「（元のラベル）確認用」を使用します。', 'alumni-core' ); ?></span></p>
	...
	public function save( $post_id, $post ) {
		...
		foreach ( $raw as $row ) {
			...
			$fields[] = array(
				...
				'textarea_rows'=>isset($row['textarea_rows']) ? max(2, min(50, absint($row['textarea_rows']))) : 4,
				'email_confirm'=>!empty($row['email_confirm']),
				'email_confirm_label'=>isset($row['email_confirm_label']) ? sanitize_text_field($row['email_confirm_label']) : '',
